...
onready var screen = get_parent().get_parent().get_parent()

signal exit
signal beverage(type)
...
class Command:
	...
	func _init(ihelp, ifunction, idefault_args=[]):
		help = ihelp
		function = ifunction
		default_args = idefault_args
	
	func execute(arg_array = default_args):
		function.call_func(arg_array)

var Commands = {
	"serve": Command.new("<beverage>			Serve drink in an empty cup. Beverage can be Tea/Koffee/StarTearsOfJoy", funcref(self,"make_drink"))
}
...
func _input(event):	
	if has_focus() && event is InputEventKey and event.pressed:
		# Cancel input
...
func parse_input(input : String):
	...
	if (split_input.size() > 0):
		var typed_command = split_input[0].to_lower()
		match (typed_command):
...
#				get_tree().quit()
				return true
			_:
				if Commands.has(typed_command):
					split_input = split_input as Array
					split_input = split_input.slice(1,split_input.size()-1)
					Commands[typed_command].execute(split_input)
					return true
...
func display_help_commands():
	...
	for command in Commands:
		draw_new_line("	[b]"+command+"[/b] "+Commands[command].help, false, true, false)
...
func draw_error(msg):
	draw_new_line("	[color=#ff1111]Error:[/color] "+msg, false, true, false)

func draw_too_any_arg(expected, got):
	draw_error("Too many arguments. Expected %d got %d" % expected  % got)

func make_drink(args = []):
	if args.size() < 1:
		draw_error("No beverage specified. (Tea/Koffee/StarTearsOfJoy)")
	elif args.size() > 1:
		draw_too_any_arg(1, args.size())
	else:
		var lower_beverage_type = args[0].to_lower()
		if lower_beverage_type in ["tea", "koffee", "startearsofjoy"]:
			if screen.mug.visible:
				if screen.mug.animation == "empty":
					emit_signal("beverage",args[0])
				else:
					draw_error("Mug full.")
			else:
				draw_error("Mug missing.")
		else:
			draw_error("Unknown beverage. (Tea/Koffee/StarTearsOfJoy)")
